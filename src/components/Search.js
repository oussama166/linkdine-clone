...
import Jack from '/public/jack-finnigan-rriAI0nhcbc-unsplash.jpg'
                ...
                <div className='flex items-start flex-col gap-4 transition-all ease-in-out'>
                    {
                        filterData(Data, searchValue).map((item, index) => (
                            <div className='flex flex-row justify-between gap-2 w-full cursor-pointer p-2 hover:bg-gray-200 transition-colors ease-in-out' key={ index }>
                                <div className='flex flex-row items-center gap-1 ml-2'>
                                    <FaSearch className='w-4 h-4 mr-1' /> {/* Adjusted the margin */ }
                                    <p className='text-black'>{ item.name }</p>
                                    <p className='text-gray-400 text-sm'>● { item.job }</p>
                                </div>
                                <img src={ Jack } alt={ item.name } className='rounded-full w-8 h-8 object-cover' /> {/* Used item.img instead of Jack */ }
                            </div>
                        ))
